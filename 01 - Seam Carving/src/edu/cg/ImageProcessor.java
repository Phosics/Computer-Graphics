...
public class ImageProcessor extends FunctioalForEachLoops {
	...
	public BufferedImage greyscale() {
		...
		forEach((y, x) -> {
			...
			int greyColor = (r * color.getRed() + g * color.getGreen() + b * color.getBlue())
					/ (rgbWeights.weightsAmount);
	...
	public BufferedImage nearestNeighbor() {
		float weightRatio = (float) inWidth / outWidth;
		float heightRatio = (float) inHeight / outHeight;
		BufferedImage ans = newEmptyOutputSizedImage();
		setForEachOutputParameters();
		
		forEach((y, x) -> {
			ans.setRGB(x, y, workingImage.getRGB(Math.round(x * weightRatio), Math.round(y * heightRatio)));
		});
